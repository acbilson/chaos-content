...
section#consultant>p:first-child {
  border-top: 0;
...
<!--
<svg height="50" width="80" style="position: absolute; right: 2%; font-size: 10px;">
  <text x="0" y="20" fill="red" transform="rotate(30 20,40)">TOP SECRET</text>
</svg>
-->
...
document.addEventListener("DOMContentLoaded", (event) => {
  var buttons = document.querySelectorAll('button.resume');

  for (var i=0; i < buttons.length; i++) {

    buttons[i].addEventListener("click", (event) => {

      var sections = document.querySelectorAll('section.resume');
      sections.forEach((section) => {
        if (section.id == event.target.name) {
